...
function EqualSplitGrp({getMembers,amount}:{getMembers: {members: {_id: Id<"users">, name: string}[]} | undefined,amount:number}) {
  
  let equalAmount = 0;
  if(getMembers?.members?.length){
    equalAmount = amount / getMembers?.members?.length;
  }

    const groupMembers = getMembers?.members.map((person) => ({
      id: person._id,
      name: person.name,
...
<>
        ...
        <div className="text-sm text-gray-500">
          Total: <span className="font-medium">$100</span>
        </div>
      </div>
